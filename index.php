<?php
/*Gets the current state for the next game by recording the player's move in a
session variable.*/
function getCurrentState($playerThrow) {
    ...
    return $currentState; }
/*Creates an array of the player's past choices after their previous move and
uses array_rand to make a weighted random selection.*/
function getCompGuess($currentState) {
    $newState = array();
    ...
    return $compGuess;}
//Takes the computer's guess and picks the appropriate countermove for compThrow
function getCompThrow($compGuess) {
    switch ($compGuess) {
        case 'rock':
            $compThrow = 'paper';
            break;
        ...
        default:
            ...
            break; }
    return $compThrow; }
//Compares both player's throws to determine the outcome of the game.
function getOutcome($compThrow, $playerThrow) {
    global $winLossDraw;
    global $drawCount;
    global $winCount;
    global $lossCount;
    if ("$compThrow" == "$playerThrow") {
        $winLossDraw = '<div class="alert alert-warning text-center lead" role="alert">It\'s a draw! Darn.</div>';
        $drawCount++;
    }
    if ($compThrow == 'rock' && $playerThrow == 'paper') {
        $winLossDraw = '<div class="alert alert-success text-center lead" role="alert">Congratulations. You win!</div>';
        $winCount++;
    }
    ...
    if ($compThrow == 'scissors' && $playerThrow == 'paper') {
        $winLossDraw = '<div class="alert alert-danger text-center lead" role="alert">Sorry. You lose!</div>';
        $lossCount++;
    }
    ...
    if ($compThrow == 'rock' && $playerThrow == 'scissors') {
        $winLossDraw = '<div class="alert alert-danger text-center lead" role="alert">Sorry. You lose!</div>';
        $lossCount++; }}
//Adds the player's move to the appropriate state/array at the end of the round.
function addMove($lastThrow, $playerThrow) {
    switch ($lastThrow) {
        case 'rock':
            $_SESSION['rockState'][$playerThrow]++;
            break;
        case 'paper':
            $_SESSION['paperState'][$playerThrow]++;
            break;
        case 'scissors':
            $_SESSION['scissorState'][$playerThrow]++;
            break;
        case 'N/A':
            $_SESSION['initialState'][$playerThrow]++;
            break;
    } }
//Prints an icon reflecting the player's choices.
function showPlayerMove($playerThrow) {
    switch($playerThrow) {
        case "rock":
            echo '<i class="fa fa-circle fa-align-center fa-5x"></i>';
            break;
        case "paper":
            echo '<i class="fa fa-paper-plane-o fa-align-center fa-5x"></i>';
            break;
        case "scissors":
            echo '<i class="fa fa-scissors fa-align-center fa-5x"></i>';
            break;
        default:
            echo 'N/A';
            break; } }
function showCompMove($compThrow) {
    switch($compThrow) {
        case "rock":
            echo '<i class="fa fa-circle fa-align-center fa-5x"></i>';
            break;
        ...
        default:
            echo 'N/A';
            break; } }
//Determines the WLD ratio
function winPercent($winCount, $drawCount, $lossCount) {
    $total = $winCount + $drawCount + $lossCount;
    $winPercent = $winCount / $total;
    $winPercent = $winPercent * 100;
    return $winPercent; }
function drawPercent($winCount, $drawCount, $lossCount) {
    $total = $winCount + $drawCount + $lossCount;
    $drawPercent = $drawCount / $total;
    $drawPercent = $drawPercent * 100;
    return $drawPercent; }
function lossPercent($winCount, $drawCount, $lossCount) {
    $total = $winCount + $drawCount + $lossCount;
    $lossPercent = $lossCount / $total;
    $lossPercent = $lossPercent * 100;
    return $lossPercent; }
//Increases throwcount by one
function throwLastThrow($playerThrow) {
    global $throwCount;
    $throwCount++;
    $_SESSION['lastThrow'] = $playerThrow; }
/*Gets player input and prints results using POST and stores results for the AI
using SESSION*/
if (isset($_POST['button'])==1) {
    session_start();
    $compThrow = $_SESSION['nextThrow'];
    $rock = $_POST['rock'];
    $paper = $_POST['paper'];
    $scissors = $_POST['scissors'];
    $lastThrow = $_SESSION['lastThrow'];
    ...
    getOutcome($compThrow, $playerThrow);
    addMove($lastThrow, $playerThrow);
    $currentState = getCurrentState($playerThrow, $_SESSION);
    $compGuess = getCompGuess($currentState);
    $_SESSION['nextThrow'] = getCompThrow($compGuess);
    $throwCount++;
    $_SESSION['lastThrow'] = $playerThrow;
}
/*Sets initial variables*/
    ...
    $winCount = 0;
    $lossCount = 0;
    $winLossDraw = '<div class="alert alert-info text-center lead" role="alert">No games played yet!</div>"';
    $playerThrow = "N/A";
    $compThrow = "N/A";
    $rock = "rock";
    $paper = "paper";
    $scissors = "scissors";
    $lastThrow = "N/A";
    $_SESSION['initialState'] = array('rock'=>1, 'paper'=>1, 'scissors'=>1);
    $_SESSION['rockState'] = array('rock'=>1, 'paper'=>1, 'scissors'=>1);
    $_SESSION['paperState'] = array('rock'=>1, 'paper'=>1, 'scissors'=>1);
    $_SESSION['scissorState'] = array('rock'=>1, 'paper'=>1, 'scissors'=>1);
    $_SESSION['lastThrow'] = 'N/A';
    $currentState = $_SESSION['initialState'];
    $compGuess = getCompGuess($currentState);
    $_SESSION['nextThrow'] = getCompThrow($compGuess);
    } ?>
...
<div class="row">
    <div class="col-md-4 col-sm-2"></div>
    <div class="col-md-4 col-sm-6">
        <form action="rpspocwork.php" method="post">
        ...
        <div class="col-md-4 col-sm-6">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tr>
                    ...
                    <td class="text-center"><?php echo $lossCount; ?></td>
                    <td class="text-center"><?php echo $drawCount; ?></td>
                </tr>
            </table>
        </div>
        <div class="col-md-4 col-sm-2"></div>
    ...
    <div class="row">
        ...
        <div class="col-md-4 col-sm-6">
            <table class="table">
                ...
                <tr>
                    <td class="text-center"><?php echo $throwCount; ?></td>
                </tr>
            </table>
        </div>
        <div class="col-md-4 col-sm-2"></div>
